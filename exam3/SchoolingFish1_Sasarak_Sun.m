function [ newPop ] = SchoolingFish1_Sasarak_Sun(newPop, inputRange, popSize, maxPopSize, filterSize, density, generation, pruneProbability)
...
for g = 1:generation
    ...
    sortedPop = sortrows(newPop, 2);
    
    % generate the "genetic" filter
    s = std(newPop, 0, 1);
    genFilter(1) = sortedPop(1, 1) - s(1);
    for i = 1:(filterSize - 2)
        genFilter(i + 1) = sortedPop(round(row * i / (filterSize - 1)), 1) - s(1);
    end
    genFilter(filterSize) = sortedPop(row, 1) - s(1);
    ...
    score = ObjectiveFunction1_Sasarak_Sun( childPop );
